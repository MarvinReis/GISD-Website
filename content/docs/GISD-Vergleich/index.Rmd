---
title: "Vergleich der GISD-Versionen"
...
# Vergleich der GISD-Versionen

Hier werden die Verschiedenen GISD-Versionen über die Jahre verglichen. Dazu werden die GISD-Scores verschiedenen Gemeinden über die Jahre 1998 bis 2017 porträtiert. Zudem wird die Lebenserwartung in den Landkreisen nach den verschiedenen Versionen des GISD im bezüglich aktuellstem Jahr dargestellt. Am Ende der Seite findet sich eine Tabelle mit Anpassungen, welche in der jeweiligen Version vorgenommen wurden.
...
kable(Tabelle_GISD_München, format = "simple", alig = 'c')
```

### Flensburg
```{r Flensburg, echo=FALSE}
...
Tabelle_GISD_Flensburg <- as_tibble(Tabelle_GISD_Flensburg)
kable(Tabelle_GISD_Flensburg, format = "simple", alig = 'c')
...
Tabelle_GISD_Jena <- as_tibble(Tabelle_GISD_Jena)
kable(Tabelle_GISD_Jena, format = "simple", alig = 'c')
```


### Pirmasens
```{r Pirmasens, echo=FALSE}
...
Tabelle_GISD_Pirmasens <- as_tibble(Tabelle_GISD_Pirmasens)
kable(Tabelle_GISD_Pirmasens, format = "simple", alig = 'c')
```


### Bautzen
```{r Bautzen, echo=FALSE}
...
Tabelle_GISD_Bautzen <- as_tibble(Tabelle_GISD_Bautzen)
kable(Tabelle_GISD_Bautzen, format = "simple", alig = 'c')
```

### Starnberg
```{r Starnberg, echo=FALSE}
...
Tabelle_GISD_Starnberg <- as_tibble(Tabelle_GISD_Starnberg)
kable(Tabelle_GISD_Starnberg, format = "simple", alig = 'c')
```
...
### Lebenserwartung der Landkreise nach dem GISD in den verschiedenen Versionen

```{r 2018, fig.cap='Version 2018 (Jahr 2014)', message=FALSE, warning=FALSE, echo=FALSE}
ggplot(dat_Leberw_2014) +
  ...
  labs(x = "GISD-Score", y = "Lebenserwartung in Jahren") +
  scale_colour_identity(name = "Geschlecht", labels = c("Männer", "Frauen"),  guide = "legend") +
  theme_rki()
```

```{r 2020, fig.cap='Version 2020 (Jahr 2017)', message=FALSE, warning=FALSE, echo=FALSE}
...
```{r 2021, fig.cap='Version 2021 (Jahr 2017)', message=FALSE, warning=FALSE, echo=FALSE}
...
```{r, fig.cap='Version 2.0 2021 (Jahr 2017)', message=FALSE, warning=FALSE, echo=FALSE}
...
Tab_Änderungen <- rbind(Tab_Änderungen, cbind("Version" = "", "Anpassungen" = "- Logarithmierung von Einkommensteuer, Bruttoeinkommen und Haushaltseinkommen"))

Tab_Änderungen <- as_tibble(Tab_Änderungen)
kable(Tab_Änderungen, format = "html")
```